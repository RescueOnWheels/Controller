...
class SteamController extends EventEmitter {
  ...
  connect() {
    if (Object.keys(platforms).indexOf(process.platform) > -1) {
      ...
      this.emit('error', {
        code: 400,
        message: 'Controller: Unsupported platform detected!',
      });
      return;
    }
    ...
    if (!this.controller_device) {
      this.emit('error', {
        code: 404,
        message: 'Controller: Steam Controller not found in the connected devices!',
      });
      return;
    }
  ...
  disconnect() {
    ...
    console.info('Controller: Closed HID of Steam Controller!');
    this.hid.close();
  }
  ...
  onData(eventData) {
    ...
    switch (eventType) {
      ...
      case STEAMCONTROLLER_EVENT_CONNECTION:
      {
        ...
        switch (details) {
          case STEAMCONTROLLER_CONNECTION_EVENT_DISCONNECTED:
            console.info('Controller: Steam Controller disconnected!');

            break;

          case STEAMCONTROLLER_CONNECTION_EVENT_CONNECTED:
            console.info('Controller: Steam Controller connected!');
            break;
          ...
          default:
            console.warn('Controller: Received unknown connection event type', details);
      ...
      default:
      {
        console.warn('Controller: Received unknown event type', eventType);
        break;
      }
    }
  }
 ...
 * all calls to `require('./Controller')` will return this same instance.
 */
